...
The docker image provided in the artifact can then be loaded with the following command:
\begin{itemize}
    \item \texttt{docker load -i webglitch\_image.tar}
\end{itemize}
...
\begin{itemize}
    \item \texttt{python3 time.py 5}
    \item \texttt{python3 coverage.py 5 -u}
\end{itemize}
\end{getting}
...
\begin{platforms}
The artifact is known to work on Ubuntu 24.04 with the following hardware configuration: Intel i7-13700K CPU, 64 GB DDR5 RAM, NVIDIA RTX 4080, and 500 GB of available disk space. 
...
The throughput analysis results presented in Section 3.7 can be reproduced by running \texttt{python3 time.py} from within the \texttt{/WebGlitch} directory.
By default, the script generates and executes 1,000 WebGPU programs, but this number can be modified by passing a second argument.
...
The total number of programs generated by the fuzzers can be set with a second argument.
Running the CTS may be time-consuming. To speed up evaluation, pre-collected WebGPU CTS coverage data can be used by passing the flag \texttt{-u} when running the command.
